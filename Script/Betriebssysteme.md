...
Kontrolle über das System und Zugriff auf alle Ressourcen des Systems. Im Gegensatz dazu erlauben zeitgenössische
Computersysteme, dass mehrere Programme gleichzeitig in den Speicher geladen und ausgeführt werden. Diese Entwicklung
erforderte eine festere Kontrolle und mehr Abgrenzung der verschiedenen Programme; und diese Bedürfnisse führten zum
...
Je komplexer das Betriebssystem ist, desto mehr wird erwartet, dass es im Namen seiner Benutzer tut. Obwohl seine
Hauptaufgabe die Ausführung von Benutzerprogrammen ist, muss es sich auch um verschiedene Systemaufgaben kümmern, die am
besten im Benutzerraum und nicht innerhalb des Kernels erledigt werden. Ein System besteht daher aus einer Sammlung von
...
Eine Frage, die sich bei der Diskussion über Betriebssysteme stellt, ist, wie man alle CPU-Aktivitäten bezeichnet. Frühe
Computer waren Batch-Systeme, die Jobs ausführten, gefolgt von der Entstehung von Time-Sharing-Systemen, die
Benutzerprogramme oder Aufgaben ausführten. Selbst in einem Einbenutzersystem kann ein Benutzer möglicherweise mehrere
Programme gleichzeitig ausführen: eine Textverarbeitung, einen Webbrowser und ein E-Mail-Programm. Und selbst wenn ein
Computer nur ein Programm gleichzeitig ausführen kann, wie bei einem eingebetteten Gerät, das Multitasking nicht
...
Obwohl wir persönlich den zeitgenössischeren Begriff Prozess bevorzugen, hat der Begriff Job historische Bedeutung, da
...
- Textabschnitt - der ausführbare Code
- Datenabschnitt - globale Variablen
- Heapabschnitt - Speicher, der während der Programmlaufzeit dynamisch zugewiesen wird
- Stackabschnitt - temporärer Datenspeicher beim Aufrufen von Funktionen (wie Funktionsparameter, Rückkehradressen und
  lokale Variablen)
  Beachten Sie, dass die Größen der Text- und Datenabschnitte fest sind, da ihre Größen während der Programmlaufzeit
  nicht ändern. Die Stack- und Heapabschnitte können jedoch während der Programmausführung dynamisch schrumpfen und
  wachsen. Jedes Mal, wenn eine Funktion aufgerufen wird, wird ein Aktivierungsdatensatz mit Funktionsparametern,
  lokalen Variablen und der Rückkehradresse auf den Stack geschoben; wenn die Kontrolle von der Funktion zurückgegeben
  wird, wird der Aktivierungsdatens
...
- Waiting. Der Prozess wartet auf das Eintreten eines Ereignisses (wie z. B. die Fertigstellung einer E/A-Operation oder
...
- Terminated. Der Prozess hat die Ausführung beendet.
  ...
  können jedoch bereit und wartend sein. Das Zustandsdiagramm, das diesen Zuständen entspricht, wird in Abbildung 3.2
  präsentiert.
...
Jeder Prozess wird im Betriebssystem durch einen Prozesssteuerungsblock (PCB) repräsentiert - auch als Task Control
Block bezeichnet. Ein PCB wird in Abbildung 3.3 gezeigt. Er enthält viele Informationen, die mit einem spezifischen
Prozess verbunden sind, einschließlich dieser:

- Prozesszustand. Der Zustand kann neu, bereit, ausführend, wartend, angehalten usw. sein.
- Befehlszähler. Der Zähler gibt die Adresse der nächsten auszuführenden Anweisung für diesen Prozess an.
- CPU-Register. Die Register variieren in Anzahl und Typ, je nach Computerarchitektur. Dazu gehören Akkumulatoren,
  Indexregister, Stackpointer und allgemeine Zweckregister sowie jegliche Zustandscode-Informationen. Zusammen mit dem
  Befehlszähler müssen diese Zustandsinformationen bei einem Unterbrechungsereignis gespeichert werden, um zu
  ermöglichen, dass der Prozess später korrekt fortgesetzt wird, wenn er erneut zur Ausführung geplant ist.
- CPU-Scheduling-Informationen. Diese Informationen umfassen eine Prozesspriorität, Zeiger auf Scheduling-Warteschlangen
  und alle anderen Scheduling-Parameter. (Kapitel 5 beschreibt die Prozessplanung.)
- Speicherverwaltungsinformationen. Diese Informationen können Elemente wie den Wert der Basis- und Begrenzungsregister
  und die Seitentabellen oder Segmenttabellen umfassen, abhängig vom vom Betriebssystem verwendeten Speichersystem (
  Kapitel 9).
...
- I/O-Statusinformationen. Diese Informationen umfassen die Liste der dem Prozess zugewiesenen I/O-Geräte, eine Liste
  offener Dateien usw.
  Kurz gesagt, dient der PCB einfach als Repository für alle Daten, die benötigt werden, um einen Prozess zu starten
...
jedem Programm interagieren können, während es läuft. Um diese Ziele zu erreichen, wählt der Prozessplaner einen
...
Wenn Prozesse in das System eintreten, werden sie in eine Bereitschaftswarteschlange eingereiht, in der sie bereit und
wartend sind, um auf einem CPU-Kern ausgeführt zu werden. Diese Warteschlange wird im Allgemeinen als verkettete Liste
gespeichert; ein Bereitschaftswarteschlangen-Header enthält Zeiger auf den ersten PCB in der Liste, und jeder PCB
enthält ein Zeigerfeld, das auf den nächsten PCB in der Bereitschaftswarteschlange zeigt.

Das System umfasst auch andere Warteschlangen. Wenn einem Prozess ein CPU-Kern zugewiesen wird, führt er eine Weile aus
und beendet schließlich, wird unterbrochen oder wartet auf das Eintreten eines bestimmten Ereignisses, wie z.B. die
...
auf das Eintreten eines bestimmten Ereignisses warten – wie die Fertigstellung von I/O – werden in eine Warteschlange
gestellt (Abbildung 3.4).

Eine gängige Darstellung der Prozessplanung ist ein Warteschlangendiagramm, wie das in Abbildung 3.5. Zwei Arten von
Warteschlangen sind vorhanden: die Bereitschaftswarteschlange und eine Reihe von Warteschlangen. Die Kreise stellen die
Ressourcen dar, die die Warteschlangen bedienen, und die Pfeile zeigen den Fluss der Prozesse im System an.

Ein neuer Prozess wird zunächst in die Bereitschaftswarteschlange gelegt. Er wartet dort, bis er zur Ausführung
...
- Der Prozess könnte eine I/O-Anfrage stellen und dann in eine I/O-Warteschlange gestellt werden.
- Der Prozess könnte einen neuen Kindprozess erstellen und dann in eine Warteschlange gestellt werden, während er auf
  die Beendigung des Kindes wartet.
- Der Prozess könnte gewaltsam vom Kern entfernt werden, als Ergebnis einer Unterbrechung oder weil sein Zeitschlitz
  abgelaufen ist, und wieder in die Bereitschaftswarteschlange gestellt werden.

In den ersten beiden Fällen wechselt der Prozess schließlich vom Wartezustand in den Bereitzustand und wird dann wieder
in die Bereitwarteschlange aufgenommen. Ein Prozess setzt diesen Zyklus fort, bis er beendet wird, woraufhin er aus
allen Warteschlangen entfernt und sein PCB (Prozesskontrollblock) sowie seine Ressourcen freigegeben werden.

### 3.2.2 CPU-Scheduling

Ein Prozess wandert während seiner Lebensdauer zwischen der Bereitwarteschlange und verschiedenen Warteschlangen. Die
Aufgabe des CPU-Schedulers besteht darin, aus den Prozessen in der Bereitwarteschlange auszuwählen und einem von ihnen
...
alle 100 Millisekunden ausgeführt, obwohl typischerweise viel häufiger.
...
Prozess zu unterbrechen und dann fortzusetzen. Der Kontext wird im PCB des Prozesses dargestellt. Er umfasst den Wert
...
Kopieren von Registerdaten in und aus dem Speicher zurück. Außerdem gilt: Je komplexer das Betriebssystem, desto größer
ist der Arbeitsaufwand, der während eines Kontextwechsels geleistet werden muss. Wie wir in Kapitel 9 sehen werden,
können fortgeschrittene Speicherverwaltungstechniken erfordern, dass zusätzliche Daten bei jedem Kontextwechsel
